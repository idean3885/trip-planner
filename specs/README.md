...
도메인 간 의존이 발생할 때의 규칙. 위반 시 도메인 경계가 무너진다.

### 원칙

1. **단방향 참조**: 화살표 방향으로만 참조. 역방향 참조 금지
2. **원천 소유권**: 데이터는 하나의 도메인만 소유. 다른 도메인은 조회만
3. **인터페이스 계약**: 도메인 간 통신은 명시적 계약(이벤트 또는 API)으로만

### 소유권 매트릭스

| 데이터 | 원천 도메인 | 참조 도메인 |
|--------|-----------|-----------|
| Trip, Day | 일정 편성 | 여행 탐색, 동행 협업, 일정 활용 |
| Activity | 일정 편성 | 예산 관리, 일정 활용 |
...
- 동행 협업이 Activity를 직접 생성/수정 (일정 편성 소유)
- 예산 관리가 Activity의 cost를 직접 변경 (일정 편성 소유)
- 일정 활용이 Day/Activity를 변경 (읽기 전용)
- 여행 탐색이 TripMember를 참조 (동행 협업 소유)
