...
명목 컬럼 제거 + MCP/REST breaking. spec 030(자동 부트스트랩)과 동시 출시.

- [x] T050 Prisma 마이그레이션 — `Trip.start_date`/`end_date` 컬럼 DROP [artifact: prisma/migrations/20260528000000_trip_period_contract/migration.sql] [why: trip-derived-contract] [migration-type: schema-only]
- [x] T050A 마이그레이션 직전 데이터 보정 검증 — raw SQL 로 명목·derived 정합 + 일정 0건 trip 수 리포트. 사고 시 rollback 절차 주석 [artifact: scripts/verify-trip-period-contract-readiness.ts] [why: trip-derived-contract] [migration-type: data-migration]
- [x] T051 Prisma schema 에서 startDate/endDate 필드 제거 [artifact: prisma/schema.prisma] [why: trip-derived-contract]
- [x] T052 MCP `list_trips`/`get_trip` 응답 출력 정정 — `_format_period` 헬퍼로 일정 0건 trip 을 "일정 미정" 으로 노출 (MCP 에 별도 `create_trip`/`update_trip` 도구는 존재하지 않아 시그니처 변경 대상이 없음, spec 의도는 응답 정합) [artifact: mcp/trip_mcp/planner.py::_format_period] [why: mcp-trip-signature-breaking]
- [x] T053 REST POST/PUT `/api/(v2/)trips/<id>` body 에서 startDate/endDate 입력 거부 (400) — MCP 가 사용하는 같은 엔드포인트가 breaking 정합 처리 [artifact: src/app/api/trips/<id>/route.ts] [why: mcp-trip-signature-breaking]
- [x] T054 OpenAPI Trip 스키마 갱신 — `startDate`/`endDate` 응답 nullable 유지 + description 에 derived 출처 명시 + createTrip/updateTrip body 의 startDate/endDate 입력 필드 제거 [artifact: src/lib/openapi.ts] [why: mcp-trip-signature-breaking]
- [x] T054A 부수 — `getResolvedPeriod` fallback 인자 제거 (DerivedPeriod 단순 반환), `expandTripRangeIfNeeded` 함수 제거 + days route 가 `getDerivedPeriodTx` 사용, trips/new 폼 startDate/endDate 입력 제거 [artifact: src/lib/trip-period.ts::getResolvedPeriod] [artifact: src/lib/day-number.ts] [artifact: src/app/trips/new/page.tsx] [why: trip-derived-contract]
