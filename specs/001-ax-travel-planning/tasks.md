...
- [ ] T022 [US3] Next.js 프로젝트 초기�� + 마크다운 렌더링 + AppPaaS 배포 (#49) — `web/` 디렉토리에 Next.js 앱 생성 (Node 20). trips/ 마크다운을 빌드 시점에 정적 페이지로 렌더링. 모바일 반응형. AppPaaS 배포 + 파이프라인 검증 (마크다운 수정 → 재배포 → 반영)

### 모바일 최적화 + 파이프라인 검증

- [ ] T025 [US3] 모바일 반응형 검증 (#32) — 웹앱을 모바일 브라우저(iOS Safari, Android Chrome)에서 열어 확인. 줌/가로스크롤 없이 열람 가능한지 테스트
- [ ] T026 [US3] 일정 수정 후 반영 검증 (#31) — 일정 수정 → 앱 재배포 → 모바일에서 새로고침하여 최신 일정 확인
