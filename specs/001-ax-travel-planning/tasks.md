...
**Goal**: 확정된 여행 요소를 기반으로 일정을 생성하고, Next.js 웹앱을 통해 동행자에게 모바일로 공유한다
...
- [ ] T019 [US2] 실제 여행 데이터로 1일차 일정 생성 테스트 — "6/7 리스본 도착일 일정 만들어줘"를 수행. `trips/2026-honeymoon-portugal-spain/daily/day01-0607-lisbon.md` 생성 확인. CLAUDE.md 포맷 준수 여부를 `scripts/validate-daily.py`로 자동 검증
- [ ] T020 [US2] 기존 예약 데이터 유지 검증 — 예약 완료 숙소 데이터가 있는 상태에서 일정 생성 요청. 기존 `✅ 예약 완료` 항목이 유지되고 미확정 부분만 새로 채워지는지 확인
- [ ] T021 [US2] 전체 일정 요약(`itinerary.md`) 생성 검증 — 전체 요약 생성 요청. 여행 루트·기간·숙소 한눈에 보기, 일별 목차(daily 파일 링크) 포함 확인

### 웹앱 구축 (AppPaaS Next.js)

> T022~T024는 기존 GitHub Pages 태스크를 AppPaaS Next.js로 재정의. 기존 GitHub Pages 작업(#34, #30, #33)은 완료되었으나 대체됨.

- [ ] T022 [US3] Next.js 프로젝트 초기화 — `web/` 디렉토리에 Next.js 앱 생성 (Node 20). AppPaaS 배포 설정
- [ ] T023 [US3] AppPaaS 배포 설정 — AppPaaS에 웹앱 배포. 도메인 확인
- [ ] T024 [US3] 마크다운 데이터 웹앱 연동 — trips/ 마크다운 파일을 Next.js에서 정적 데이터로 렌더링. 일별 일정 페이지 라우팅

### 모바일 최적화

- [ ] T025 [US3] 모바일 반응형 검증 — 웹앱을 모바일 브라우저(iOS Safari, Android Chrome)에서 열어 확인. 줌/가로스크롤 없이 열람 가능한지 테스트. 문제 발견 시 레이아웃 조정
- [ ] T026 [US3] 일정 수정 후 반영 검증 — 일정 수정 → 앱 재배포 → 모바일에서 새로고침하여 최신 일정 확인

**Checkpoint**: US2+US3 독립 검증 완료. 일정 생성 → 웹앱 딜리버리 → 모바일 열람 전체 흐름 확인
...
```
T019~T021 (일정 생성 검증) + T022~T024 (웹앱 구축) → T025~T026 (모바일 검증)
```
...
```bash
# M2: CLI 구현 내 병렬
T010 (locations) | T011 (attractions) | T012 (details)  # T009 완료 후

# M3: 일정 생성 + 웹앱 병렬
T019~T021 (일정 검증) | T022~T024 (웹앱 구축)
```
...
- T022~T024는 기존 GitHub Pages 태스크를 AppPaaS Next.js로 재정의 (이슈 내용 갱신 필요)
