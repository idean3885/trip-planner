...
## Clarifications *(resolved 2026-04-17)*

스펙 초안 검토에서 드러난 5개 모호 지점을 결정으로 봉합한다. 각 결정은 WHAT/WHY 수준이며, 구체 구현 수단은 plan 단계에서 결정한다.

1. **tasks.md 아티팩트 선언 형식** — 각 태스크 항목은 산출 아티팩트를 `[artifact: <식별자>]` 메타태그로 명시한다. 식별자는 (a) 레포 상대 경로 또는 (b) 파일 내 고유 심볼(`path::symbol`)을 사용한다. 검증기는 메타태그 문자열과 실제 파일 트리·심볼 색인 존재 여부만 확인하며, 내용 정합성은 판단하지 않는다.
2. **drift 감사 실행 주기/위치/리포트** — 기본 주기는 매주 월요일 09:00 KST. 실행 위치는 원격 CI(스케줄 실행)이며 개발자 로컬 훅은 사용하지 않는다(스케줄 일관성). 리포트는 `docs/audits/drift/YYYY-MM-DD.md`에 누적 기록한다. 주기·위치·경로는 설정으로 오버라이드 가능.
3. **quickstart 증거 저장·연결 방식** — `quickstart.md`를 피처 디렉토리 내 별도 파일로 둔다. 시나리오 섹션마다 `### Evidence` 서브섹션을 두고 (a) 자동 테스트 파일 상대 경로 또는 (b) 수동 체크리스트 + 스크린샷 경로(`docs/evidence/<feature>/<scenario>-*.png`)를 기록한다. PR 본문에는 `quickstart.md` 링크만 포함한다.
4. **Why 동등성 판단 기준** — tasks.md의 각 태스크는 `[why: <짧은-태그>]` 메타태그를 부착한다. 동일 태그를 가진 태스크만 이슈 합산 대상이다. 태그 네이밍 정합성 검증만 자동화하고, 의미적 유사도 판정은 수행하지 않는다(휴리스틱 오탐 차단).
5. **plan bullet 다단 감지 기준** — plan 작성자가 다단 성격의 bullet에 `[multi-step: N]` 태그(N=최소 필요 태스크 수)를 명시한다. 태그가 있는 bullet은 tasks.md에 `N`개 이상의 태스크로 매핑되지 않으면 차단된다. 자동 감지(NLP 추정)는 시도하지 않는다(거짓 음성 위험).

메타태그 표기법 결정이 본 스펙의 근간이므로, plan은 이 표기를 전제로 탐색·검증 로직을 설계한다.
...
- **FR-002**: 시스템은 plan bullet에 `[multi-step: N]` 태그가 명시된 경우, tasks.md에 해당 bullet을 참조하는 태스크가 `N`개 이상 존재하는지 검증해야 한다. 부족하면 차단한다.
- **FR-003**: 시스템은 PR 단계에서 tasks.md 체크박스 상태와 각 태스크의 `[artifact: ...]` 메타태그가 가리키는 레포 아티팩트 일치성을 교차 검증하여, 체크됐으나 아티팩트가 없으면 에러, 미체크인데 아티팩트가 있으면 경고를 출력해야 한다.
- **FR-004**: 시스템은 활성 피처에 대해 설정 가능한 주기(기본: 매주 월요일 09:00 KST)로 drift 감사 리포트를 `docs/audits/drift/YYYY-MM-DD.md`에 생성해야 한다. 실행 위치는 원격 CI 스케줄 실행이며 개발자 로컬 훅은 사용하지 않는다.
- **FR-005**: 시스템은 `quickstart.md`의 각 시나리오 섹션에 `### Evidence` 서브섹션이 존재하고, 자동 테스트 파일 상대 경로 또는 수동 체크리스트 + 스크린샷 경로 중 최소 하나가 기록되어 있는지 PR 머지 게이트에서 검증해야 한다.
- **FR-006**: 시스템은 plan에 DB 스키마 변경 또는 enum 값 추가가 명시된 경우, tasks.md에 대응 데이터 보정 태스크(별도 `[artifact: ...]` 및 `[why: ...]` 태그를 가진) 존재를 강제해야 한다.
- **FR-007**: 시스템은 마이그레이션 산출물에 `[migration-type: schema-only|data-migration]` 구분 메타데이터를 요구해야 한다.
...
- **FR-011**: 시스템은 tasks → 이슈 전사 시 동일한 `[why: <태그>]` 메타태그를 가진 태스크를 하나의 이슈로 합산하고, 합산 이슈의 추정 공수가 8h를 초과하면 이슈를 분할해야 한다.
...
- **FR-015**: drift 감사 주기, 실행 위치, 감사 리포트 보관 경로는 단일 설정 파일에서 오버라이드 가능해야 한다.
- **FR-016**: 시스템은 speckit 템플릿(tasks, plan)에 `[artifact]`, `[why]`, `[multi-step]`, `[migration-type]` 메타태그 사용법 가이드를 포함해야 한다.
- **FR-017**: 시스템은 `[artifact]`/`[why]`/`[multi-step]`/`[migration-type]` 메타태그의 **형식 정합성**(괄호 매칭, 값 유효성)만 검증하고, 의미적 유사도·NLP 기반 자동 추정은 수행하지 않는다.

### Key Entities *(include if feature involves data)*

- **피처 산출물**: spec.md, plan.md, tasks.md, quickstart.md(별도 파일), 마이그레이션 산출물 — 각각 피처 디렉토리 내 정해진 위치에서 탐색된다.
- **메타태그**: 산출물 본문에 삽입되는 구조화 마커. `[artifact: <식별자>]`, `[why: <태그>]`, `[multi-step: N]`, `[migration-type: schema-only|data-migration]` 네 종이며, 검증기가 정합성 판단의 기본 단위로 사용한다.
- **커버리지 맵**: plan.md bullet ↔ tasks.md 태스크 간의 다대다 매핑 집합. bullet의 `[multi-step: N]` 태그 유무에 따라 최소 매핑 수가 결정된다.
- **drift 리포트**: 시점별 tasks ↔ artifact 불일치 목록(에러/경고 등급 포함). `docs/audits/drift/YYYY-MM-DD.md`에 누적.
- **quickstart 증거**: 시나리오 단위 `### Evidence` 서브섹션 (자동 테스트 경로 또는 수동 체크리스트·스크린샷 쌍).
- **마이그레이션 메타데이터**: 마이그레이션 산출물별 `[migration-type]` 라벨.
- **합산 이슈 묶음**: 이슈 전사 결과 이슈·마일스톤·`[why]` 태그 기준 묶음 관계.
...
- drift 감사가 참조할 아티팩트 선언은 tasks.md의 `[artifact: ...]` 메타태그로 제공되며, 형식은 본 스펙 Clarifications #1로 규정한다.
...
- 원격 스케줄 실행 환경이 존재하며, 설정 파일에서 주기·경로를 오버라이드 가능하다(구체 구현은 plan에서 결정).
