...
- 템플릿 경로: `.specify/templates/implement-template.md` (#212에서 신규 작성)
- 자동 테스트 경로: 해당없음 — implement는 템플릿 로드 단계라 수동 확인 기준
- 수동 체크리스트:
  - [x] US6-1 — 레이어 테이블(도메인/애플리케이션/인프라/표현) 포함 확인 (#212 PR 템플릿 본문)
  - [x] US6-2 — "Breaking Change 검증" 섹션에 expand/migrate/contract 체크리스트 포함 확인 (#212 PR)
  - [x] US6-3 — Git Flow Lite 커밋 규칙(merge commit 고정) 포함 확인 (#212 PR)
  - [x] US6-4 — 테스트 전략 표(변경 유형 → 최소 테스트 조합) 포함 확인 (#212 PR)
- 스크린샷: 해당없음(템플릿 마크다운 내용이 증거)
