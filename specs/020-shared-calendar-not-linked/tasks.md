...
- [ ] T008 [P] 단위 테스트 작성 — `GET /api/trips/[id]/gcal/status`가 `TripCalendarLink` 부재 시 본인 `GCalLink` 존재 여부와 **무관하게** `{ linked: false, scopeGranted }`를 반환함을 확인(레거시 폴백 회귀 방지). Prisma mock 사용 [artifact: tests/api/gcal-status.test.ts] [why: test-coverage] <!-- v2.9.1 릴리즈 시점 미작성 — #407 정리에서 추적 -->
...
- [X] T011 changes 단편 작성 — What/Why 2줄 원칙으로 본 피처의 사용자 가시 변경을 기술한다. 이슈 번호는 PR 직전 생성 후 파일명에 반영(`changes/<이슈번호>.fix.md`) [artifact: CHANGELOG.md] [why: status-fallback-removal]
