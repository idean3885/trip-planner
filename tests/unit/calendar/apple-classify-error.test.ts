...
describe("appleProvider.classifyError — 6종 vocabulary 매핑", () => {
  ...
  it("status=412 → precondition_failed", () => {
    expect(appleProvider.classifyError({ status: 412 })).toBe(
      "precondition_failed",
    );
  });

  it("status=429 → transient_failure", () => {
    expect(appleProvider.classifyError({ status: 429 })).toBe(
      "transient_failure",
    );
  });

  it("status=500 → transient_failure", () => {
    expect(appleProvider.classifyError({ status: 503 })).toBe(
      "transient_failure",
    );
  });
  ...
  it("Error message에 ECONNREFUSED → transient_failure", () => {
    expect(
      appleProvider.classifyError(new Error("connect ECONNREFUSED 127.0.0.1")),
  ...
  it("Error message에 '401' 포함 시 status 추출", () => {
    expect(
      appleProvider.classifyError(new Error("HTTP 401 Unauthorized")),
    ).toBe("auth_invalid");
  });

  it("status 추출 불가 + 네트워크 키워드 부재 → null", () => {
    expect(
      appleProvider.classifyError(new Error("totally unknown error")),
    ).toBeNull();
  });
