...
describe("classifyError", () => {
  it("401/403 → forbidden + REVOKED", () => {
    expect(classifyError({ code: 401 })).toEqual({
      reason: "forbidden",
      lastError: "REVOKED",
    });
    expect(classifyError({ code: 403 })).toEqual({
      reason: "forbidden",
      lastError: "REVOKED",
    });
  });
  ...
  it("404 → not_found, lastError null (조용한 정리)", () => {
    expect(classifyError({ code: 404 })).toEqual({
      reason: "not_found",
      lastError: null,
    });
  });
  ...
  it("5xx → network + NETWORK", () => {
    expect(classifyError({ code: 500 })).toEqual({
      reason: "network",
      lastError: "NETWORK",
    });
    expect(classifyError({ code: 503 })).toEqual({
      reason: "network",
      lastError: "NETWORK",
    });
  });
  ...
  it("네트워크 오류(status=0) → network", () => {
    expect(classifyError(new Error("connection refused"))).toEqual({
      reason: "network",
      lastError: "NETWORK",
    });
  });
  ...
  it("분류 불가 → unknown", () => {
    expect(classifyError({ code: 418 })).toEqual({
      reason: "unknown",
      lastError: "UNKNOWN",
    });
  });
