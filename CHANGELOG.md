...
- **캘린더를 글래스 테마에 맞춰 다듬었습니다.** 선택한 날짜와 오늘은 솔리드 박스 대신 반투명 톤과 얇은 브랜드 링으로, 여행 기간은 검은 밑줄 대신 반투명 밴드로 보입니다. 월 달력의 셀 높이를 줄여 아래 빈 공간을 없앴습니다. 더불어 유리 카드의 좌우 테두리가 깨져 보이던 렌더링 문제(반투명 효과와 모서리 처리 충돌)를 고쳤습니다. 요일 색 구분과 팔레트는 그대로 유지합니다. ([#932](https://github.com/idean3885/trip-planner/issues/932))

### Fixed

- **여행 상세 화면의 자잘한 문제들을 손봤습니다.** "여행 목록" 브레드크럼이 첫 화면 대신 여행 목록으로 바르게 이동하고, 카드 테두리가 좌우·위쪽에서 끊겨 보이던 문제를 고쳤습니다. 캘린더는 흰 배경 상자를 걷어 화면에 자연스럽게 녹아들게 하고 세로 높이를 줄였으며, 일정이 있는 날은 선 대신 점으로, 여행 기간은 양 끝만 둥근 이어진 띠로 표시합니다. ([#934](https://github.com/idean3885/trip-planner/issues/934))
- **여행 상세 화면의 섹션들을 하나의 유리 톤으로 통일했습니다.** 상단 브레드크럼·캘린더·일정 영역이 제각각 보이던 것을 같은 반투명 유리 표면으로 맞췄습니다. 또 캘린더 아래에 떠 있던 빈 공간을 없앴습니다(달력 높이를 현재 달에 맞도록 조정). 빈 날짜의 "일정 없음" 안내도 같은 유리 카드로 바꿨습니다. ([#936](https://github.com/idean3885/trip-planner/issues/936))
- **캘린더가 유리 테마에 제대로 녹아들도록 고쳤습니다.** 캘린더 안에 흰 배경이 한 겹 더 깔려 유리 효과를 가리던 문제를 없애 다른 카드와 같은 톤이 되게 하고, 여행 기간 표시 색을 또렷하게 올렸으며 선택한 날짜의 모서리 둥글기를 나머지와 맞췄습니다. 데스크탑 캘린더에도 같은 유리 표면을 씌웠습니다. 또 상단 헤더 바와 그 아래 이동 경로 바를 하나로 이어 붙여 두 개로 갈라져 보이던 것을 정리했습니다. ([#938](https://github.com/idean3885/trip-planner/issues/938))
- **캘린더 표시를 다듬었습니다.** 일정이 있는 날의 점이 파랑 계열과 겹쳐 잘 안 보이던 것을 코랄색으로 바꿔 또렷하게 했습니다. 여행 기간 표시가 날짜마다 끊겨 보이던 것을 가운데는 이어지고 양 끝만 둥글게 정리했으며, 선택한 날짜의 강조 영역이 날짜에 비해 너무 크던 것도 줄였습니다. ([#940](https://github.com/idean3885/trip-planner/issues/940))
- **여행 메뉴(☰) 버튼을 눌렀을 때 목록이 달력 뒤로 숨던 문제를 고쳤습니다.** 메뉴가 항상 앞에 뜨도록 층 순서를 바로잡았습니다. ([#942](https://github.com/idean3885/trip-planner/issues/942))
...
- **여행 상세 화면까지 글래스모피즘을 넓혔습니다.** 활동 카드와 주간 달력이 반투명 유리로 보여, 가장 자주 보는 화면에서도 유리 질감이 드러납니다. 더불어 카드 테두리를 네 변 고르게 선명하게 다듬어, 좌우가 흐려 깨져 보이던 인상을 없앴습니다. ([#928](https://github.com/idean3885/trip-planner/issues/928))
...
- 대문과 소개 화면에서 겹치던 안내를 정리했습니다. 같은 버튼과 링크가 여러 번 반복돼 어수선했기 때문입니다.
  ...
  * 소개 화면을 만든 이유·만든이·저장소 중심으로 줄이고, 설치·사용 방법은 안내 문서 링크로 옮겼습니다.
  ...
  ([#894](https://github.com/idean3885/trip-planner/issues/894))
...
- 로그인한 뒤에도 대문(소개 화면)을 볼 수 있게 했습니다. 로그인 상태에서는 대문이 여행 목록으로 곧장 넘어가 버려 소개 화면을 다시 열 수 없었기 때문입니다.
  ...
  * 대문에서 같은 취지로 겹치던 소개 영역을 하나로 합쳤습니다.
  ...
  ([#889](https://github.com/idean3885/trip-planner/issues/889))

### Chore

- UX 리뷰 전문가 검토 창구를 프로젝트에 추가했습니다. 화면·플로우 변경 시 정보구조·카피·접근성을 일관된 기준으로 되짚어, 사용자 경험 판단의 근거를 남기기 위함입니다. ([#888](https://github.com/idean3885/trip-planner/issues/888))
