...
import { onMemberLeave, onRoleChange } from "@/lib/gcal/member-sync";
...
export async function PATCH(request: Request, { params }: Params) {
  ...
  if (action === "promote") {
    ...
    if (target.role !== "GUEST") {
      return NextResponse.json(
        { error: "게스트만 승격할 수 있습니다" },
        { status: 400 },
      );
    }
    if (!(await isHost(tripId, userId))) {
      return NextResponse.json(
        { error: "호스트만 승격할 수 있습니다" },
        { status: 403 },
      );
    }
    ...
    if (target.role !== "HOST") {
      return NextResponse.json(
        { error: "호스트만 강등할 수 있습니다" },
        { status: 400 },
      );
    }
    if (!(await isOwner(tripId, userId))) {
      return NextResponse.json(
        { error: "주인만 강등할 수 있습니다" },
        { status: 403 },
      );
    }
    await prisma.tripMember.update({
