...
2. **AI 에이전트** — Claude Code·v0.dev·ops-agent:flow 등. "모호한 흐름에서 1차 컨텍스트로 참조".
...
| **AI 에이전트** | 핸드오프 변환, 구현, 리뷰, 문서화 보조. Claude Code(구현·리뷰), v0.dev(컴포넌트 생성), ops-agent:flow(이슈→브랜치→PR 자동화), 여러 MCP 서버(trip, 사내 도구 MCP, ssh-tools 등). | 개발자가 게이트로 동작하는 한 실코드 수정 허용. 최종 머지·배포 결정은 개발자. |
...
이슈 생성 (Bug/Feat/Chore/Handoff)    ← GitHub Issue Templates
        ↓
워크트리 + 브랜치 분기                 /ops-agent:flow 또는 수동
...
| **ops-agent:flow** | 이슈 → 브랜치 → PR 파이프라인 | `/ops-agent:flow` 슬래시 명령(Claude Code 내). 피처 브랜치 네이밍 규약에 주의. |
| **MCP 서버 (trip)** | DB 여행 데이터 조회/편집 | Claude Desktop·Code의 MCP 연동. 주로 검증·조회용. |
| **MCP 서버 (기타)** | 사내 협업 도구, SSH 도구 등 | 필요 시 사용. 본 프로젝트 릴리즈 흐름엔 비핵심. |
