...
| **AI 에이전트** | 핸드오프 변환, 구현, 리뷰, 문서화 보조. Claude Code(구현·리뷰), v0.dev(컴포넌트 생성), ops-agent:flow(이슈→브랜치→PR 자동화), 여러 MCP 서버(trip, dooray-api, ssh-tools 등). | 개발자가 게이트로 동작하는 한 실코드 수정 허용. 최종 머지·배포 결정은 개발자. |
