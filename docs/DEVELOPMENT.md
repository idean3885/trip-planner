...
> **대상 독자**: 기여자·개발자 — 로컬 환경을 세팅하고 코드를 수정·기여하려는 분.
...
- **인증**: Google OAuth (웹), OAuth CLI 자동 발급·디바이스 인증 (외부 클라이언트), PAT 수동 발급 (고급·폴백)
...
**전 방향 `Create a merge commit`**. Squash and merge는 off(repo 설정으로 고정, #218).

| PR 방향 | 머지 방식 | 이유 |
|---------|----------|------|
| feature/hotfix → develop | **Create a merge commit** | 커밋 정리는 개발자가 feature 브랜치에서 직접(push 전 `git rebase -i` 등). AI 하네스로 커밋을 관리하는 상황에서 웹의 수동 squash는 단순 합치기일 뿐 메시지가 의미에 맞게 재구성되지 않음 |
